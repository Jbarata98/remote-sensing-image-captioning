...
class finetune():
    ...
    def _setup_train(self):
        if os.path.exists(checkpoint_encoder_path):
            ...
            optimizer = get_optimizer(OPTIMIZER)(
                params=filter(lambda p: p.requires_grad, image_model.parameters()),
                lr=encoder_lr) if self.enable_finetuning else None
        self.model = image_model.to(device)
    ...
    def _save_checkpoint_encoder(self, epoch, epochs_since_improvement, val_loss,
                                is_best):
