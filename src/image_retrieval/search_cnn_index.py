    ...
    def _get_image(self, display=False, classifier=None, mapping=None):
        ...
        for file_index in self.neighbors[0]:
            ...
            if self.intra_class:
                ...
                if image_n_label.get(self.index_dict[file_index])["Label"] == list(mapping.keys())[
                    list(mapping.values()).index(self.label)]:
                    # print("Not Airport")
...
def create_mappings(nr_inputs=1, intra_class=False):
    ...
    for split in splits:
        ...
        for img_name, feature in tqdm(features_list.items()):
            ...
            ref_img, target_imgs = search._get_image(display=False, classifier=classifier if intra_class else None,
                                                     mapping=id_class_mapping if intra_class else None)
            img_names_dict = {}
