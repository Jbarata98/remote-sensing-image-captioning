...
class CreateInputPegasus:
    ...
    def __init__(self, captions_name, split, hashmap_name, paths_name='../../../' + PATHS._get_captions_path(), model=AuxLM):
    ...
    def _concat_convert(self):
        ...
        for i, path in enumerate(paths_list):
            ...
            captions_dict[path] = [hashmap.get(str(tok)) for tok in list(itertools.chain.from_iterable(captions[i:i + 5])) if tok not in special_tokens] + [AuxLM_tokenizer.eos_token_id]
...
if __name__ == '__main__':
    ...
    base_filename = DATASET + '_' + str(5) + '_cap_per_img_' + str(
        int(h_parameter['min_word_freq'])) + '_min_word_freq'
    ...
    for split in splits:
        ...
        tokenizer = CreateInputPegasus(captions_name=captions_name, split=split,hashmap_name = hashmap_name, paths_name=paths_name)
