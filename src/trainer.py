...
if ARCHITECTURE == ARCHITECTURES.BASELINE.value:
    ...
    if AUX_LM == AUX_LMs.GPT2.value:
        _train = TrainGPT2(language_aux=AUX_LM, fine_tune_encoder=False, checkpoint=None)
    elif AUX_LM == AUX_LMs.PEGASUS.value:
        _train = TrainPegasus(language_aux=AUX_LM, fine_tune_encoder=False, checkpoint=None)
...
_train._load_weights_from_checkpoint(Setters()._set_paths(), _train.decoder, _train.decoder_optimizer, _train.encoder,
